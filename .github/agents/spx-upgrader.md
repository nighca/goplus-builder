...
- Update `@xgo-pkgs/spx` in `spx-gui/package.json` so it matches the target
...
- Execute `bash spx-gui/link-spx.sh` to expose the installed runtime assets under `spx-gui/public/`
