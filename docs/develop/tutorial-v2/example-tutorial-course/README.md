...
`main.gox` is the conventional entry file for the Course-author-written XGo program. `project/` is an ordinary serialized SPX project that becomes an ownerless in-memory project while the learner works. The root `assets/` contains Course-local resources addressed by the Tutorial program; `assets/videos/step-to/index.json` declares the video resource and points to `step-to.mp4` relative to its own directory. It is independent of `project/assets/`, which belongs to the embedded SPX project.
