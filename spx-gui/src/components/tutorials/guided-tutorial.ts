...
const guidedTutorialKey: InjectionKey<GuidedTutorial> = Symbol('guided-tutorial')

export function useGuidedTutorial() {
  const tutorial = inject(guidedTutorialKey)
  if (tutorial == null) throw new Error('GuidedTutorial not provided')
  return tutorial
}

export function provideGuidedTutorial(tutorial: GuidedTutorial) {
  provide(guidedTutorialKey, tutorial)
}

export type GuidedTutorialTopic = Topic & {
